                        ...
                        <div class="card" style="height: auto">
                            ...
                            <div class="card-body table-responsive table-bordered table-invoice p-0 mb-0 scrollbar-macosx" >
                                ...
                                <tbody slot="body" slot-scope="{displayData}">
                                    ...
                                    <tr v-for="row in displayData" :key="row.id" class="my-2">
                                ...
                                <div class="row justify-content-between">
                                    <div class="col-md-auto col-12 text-center pb-0 pt-0 pl-3">
                                        ...
                                        <select class="form-control-sm" v-model.number="pagination" type="number" style="width: 60px; border: 2px solid #39af21"  @change="scroll">
                                            <option>5</option>
                                            <option>50</option>
                                            <option :value="tupoksijfts.tupoksi.length">all</option>
                                        </select>
                                        from {{ totalItem }}
                                    </div>
            ...
            <div class="modal-dialog modal-dialog-centered " role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" v-if="!editmode" id="exampleModalCenterTitle">Tambah Tupoksi JFT Baru</h5>
                        <h5 class="modal-title" v-if="editmode" id="exampleModalCenterTitle">Update Tupoksi JFT</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form @submit.prevent ="editmode ? updateTupoksiJFT() : createTupoksiJFT()">
                    ...
                    <div class="modal-footer">
                        <div>
                            ...
                            :loading="btnload" :disabled="btnload" v-shortkey.focus="['ctrl', 'enter']" @shortkey="updateTupoksiJFT()">Update</v-btn>
                            <v-btn v-if="!editmode" color="primary" type="submit" class="text-capitalize" @click="date1 = false, date2 = false"
                            :loading="btnload" :disabled="btnload" v-shortkey.focus="['ctrl', 'enter']" @shortkey="createTupoksiJFT()">Simpan</v-btn>
                        </div>
                    </div>
                    </form>
                </div>
    ...
    export default {
        data() {
          return {
                ...
                pagination: 5,
                filters: {
                    name: { value: '', keys: ['butir_kegiatan'] }
                },
        ...
        methods:{
            filterJabatan(){
                ...
                .then(( data ) => (this.tupoksijfts = data.data));
            },
            toggleSelect() {
            ...
            editModal(tupoksijfts){
                this.editmode = true;
                this.form.reset();
                this.form.errors.errors = {};
                $('#formTupoksi').modal('show');
                this.form.fill(tupoksijfts);
            },
            addModal(){
                this.editmode = false;
                this.form.errors.errors = {};
            ...
            loadTupoksi(){
                if(this.$gate.isSAandSYS() || this.$gate.isUP()){
                    this.$Progress.start();
                    axios.get("api/tupoksibyid/"+this.tupoksijfts.id).then(( data ) => (this.tupoksijfts = data.data));
                    this.$Progress.finish();
                }
            },
            loadPosition(){
                if(this.$gate.isSAandSYS() || this.$gate.isUP()){
                    axios.get("api/jabatan-jft").then(( data ) => (this.positions = data.data));
                }
            },
            ...
            createTupoksiJFT(){
                this.$Progress.start();
                this.btnload = true;
                this.form.id_jabatan = this.tupoksijfts.id;
